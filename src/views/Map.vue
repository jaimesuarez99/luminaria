...
<script>
import { getZoneInfo, getDevicesByType, getDeviceInfo } from '@/utils/getSalentoDevices';
  ...
  setup() {
    ...
    const setupMap = async () => {
      ...
      if (zoneInfo && zoneInfo.center_lat && zoneInfo.center_lon) {
        center.value = { lat: parseFloat(zoneInfo.center_lat), lng: parseFloat(zoneInfo.center_lon) };
      }
    ...
    const setInfoCardPosition = (position) => {
      ...
      const worldCoordinate = map.getProjection().fromLatLngToPoint(new google.maps.LatLng(position.lat, position.lng));
